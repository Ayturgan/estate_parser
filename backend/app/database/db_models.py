from sqlalchemy import (
    Column, Integer, String, Float, Boolean, DateTime, ForeignKey, Text, Index, UniqueConstraint, LargeBinary
)
...
class DBRealtor(Base):
    ...
    total_ads_count = Column(Integer, default=0, index=True)
    active_ads_count = Column(Integer, default=0, index=True)
    ...
    favorite_districts = Column(JSONB, nullable=True)  # Список районов с количеством
    property_types = Column(JSONB, nullable=True)  # Статистика по типам недвижимости
    
    # Связи
    ads = relationship("DBAd", back_populates="realtor")
    unique_ads = relationship("DBUniqueAd", back_populates="realtor")
    
    created_at = Column(DateTime, default=datetime.utcnow, index=True)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    def __repr__(self):
        return f"<Realtor(id={self.id}, phone='{self.phone_number}', name='{self.name}', ads_count={self.total_ads_count})>"
...
class DBAd(Base):
    ...
    land_area_sotka = Column(Float, nullable=True, index=True)  # Площадь участка в сотках
    ...
    property_type = Column(String, nullable=True, index=True)  # квартира, дом, участок, другое
    property_origin = Column(String, nullable=True, index=True)  # новостройка, вторичка, неизвестно
    listing_type = Column(String, nullable=True, index=True)  # продажа, аренда
    
    # Связь с риэлтором
    realtor_id = Column(Integer, ForeignKey('realtors.id', ondelete='SET NULL'), nullable=True, index=True)
    ...
    # Связь с риэлтором
    realtor = relationship("DBRealtor", back_populates="ads")
...
class DBUniqueAd(Base):
    ...
    realtor = relationship("DBRealtor", back_populates="unique_ads")
...
class DBAdDuplicate(Base):
    ...
    characteristics_similarity = Column(Float, nullable=True)  # НОВОЕ ПОЛЕ: схожесть по характеристикам недвижимости
...
class DBSettings(Base):
    ...
    category = Column(String(50), nullable=True, index=True)  # automation, scraping, processing, etc.
    is_public = Column(Boolean, default=False)  # доступно ли через публичный API
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    def __repr__(self):
        return f"<Settings(id={self.id}, key='{self.key}', value='{self.value}', type='{self.value_type}')>"
