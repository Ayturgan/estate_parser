...
import json
from app.core.config import ELASTICSEARCH_HOSTS, ELASTICSEARCH_INDEX
...
class ElasticsearchService:
    ...
    def __init__(self, hosts: List[str] = None, index_name: str = None):
        self.hosts = hosts or [ELASTICSEARCH_HOSTS]
    ...
    def index_ad(self, ad_data: Dict) -> bool:
        ...
        try:
            ...
            doc_data = {
                ...
                'land_area_sotka': ad_data.get('land_area_sotka'),
    ...
    def search_by_land_area_sotka_range(self, min_sotka: Optional[float] = None, max_sotka: Optional[float] = None, size: int = 10) -> Dict:
        """Поиск объявлений по диапазону площади участка в сотках"""
        s = Search(using=self.client, index=self.index_name)
        sotka_range = {}
        if min_sotka is not None:
            sotka_range['gte'] = min_sotka
        if max_sotka is not None:
            sotka_range['lte'] = max_sotka
        
        if sotka_range:
            s = s.query('range', land_area_sotka=sotka_range)
